...
## Part 0: Understanding Claude's Tools

Say: "Before we dive in, let me explain the tools I have available. Understanding these helps you write better CLAUDE.md files and skills."

### File Operations

| Tool | What I Do With It | Example |
|------|-------------------|---------|
| **Read** | Read any file's contents | Reading your code, configs, docs |
| **Write** | Create new files | Creating CLAUDE.md, new components |
| **Edit** | Surgically modify files | Fixing bugs, adding imports |
...
| Tool | What I Do With It | Example |
|------|-------------------|---------|
| **Bash** | Run terminal commands | `npm install`, `git status`, running tests |

Say: "I can run most terminal commands, but I'm careful with destructive ones. I'll always ask before doing something risky."

### Research & Exploration

| Tool | What I Do With It | Example |
|------|-------------------|---------|
| **Task (Explore)** | Deep codebase exploration | "Find all places errors are handled" |
| **WebFetch** | Fetch web content | Reading documentation |
| **WebSearch** | Search the internet | Finding current best practices |

### User Interaction

| Tool | What I Do With It | Example |
|------|-------------------|---------|
| **AskUserQuestion** | Get your input | Choosing between approaches |
| **TodoWrite** | Track task progress | Complex multi-step work |
...
└─────────────────────────────────────────────────────────┘
                           ...
                           ▼
            ┌──────────────────────────┐
            │   Need to READ files?    │──Yes──▶ Read, Glob, Grep
            └──────────────────────────┘
                           │ No
                           ▼
            ┌──────────────────────────┐
            │   Need to MODIFY files?  │──Yes──▶ Edit (existing) or Write (new)
            └──────────────────────────┘
                           │ No
                           ▼
            ┌──────────────────────────┐
            │   Need to RUN commands?  │──Yes──▶ Bash
            └──────────────────────────┘
                           │ No
                           ▼
            ┌──────────────────────────┐
            │   Complex exploration?   │──Yes──▶ Task (Explore agent)
            └──────────────────────────┘
                           │ No
                           ▼
            ┌──────────────────────────┐
            │   Need user decision?    │──Yes──▶ AskUserQuestion
            └──────────────────────────┘
```

### Why This Matters for Your Project

Say: "When you write CLAUDE.md and skills, knowing my tools helps you:"

1. **Give me the right context** - I can read files, so you don't need to paste code
2. **Trust me to explore** - I can search your codebase to understand it
3. **Know when I'll ask** - I ask for confirmation on risky operations
4. **Write effective skills** - Skills can leverage all these tools

Ask: "Want to see these tools in action? I can demonstrate any of them, or we can continue with the setup."
